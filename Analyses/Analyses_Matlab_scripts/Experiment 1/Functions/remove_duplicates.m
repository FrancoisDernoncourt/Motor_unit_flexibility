...
function [Pulsenew, distimenew, duplicatesList, removedList] = ...
    remove_duplicates(PulseT, distime, distime2, maxlag, jitter, tol, fsamp)
...
while ~isempty(distimmp)
    ...
    duplicatesList{i} = duplicates+i-1;
    removedList{i} = [];
    ...
    [~, survivor] = min(CoV);
    if numel(CoV) > 1
        disp("Duplicated MU found");
        temp_duplicates = [i,duplicatesList{i}];
        which_one_to_remove_temp = ones(length(temp_duplicates),1);
        which_one_to_remove_temp(survivor) = 0;
        removedList{i} = temp_duplicates(find(which_one_to_remove_temp));
    end
